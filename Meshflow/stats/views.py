...
def node_packet_stats(request, node_id: int):
    ...
    # Get interval stats
    stats = packets.annotate(
        interval_start=Trunc('first_reported_time', interval_type, **get_interval_trunc_kwargs(interval_type, interval))
    ).values('interval_start').annotate(
