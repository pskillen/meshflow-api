...
def test_constellation_serializer_valid_data(create_constellation):
    ...
    assert "bot_default_ignore_meshtastic_portnums" in data
    assert "bot_default_meshtastic_hop_limit" in data
...
def test_constellation_serializer_bot_defaults(create_constellation):
    ...
    constellation = create_constellation()
    constellation.bot_default_ignore_meshtastic_portnums = "345,ROUTING_APP"
    constellation.bot_default_meshtastic_hop_limit = 5
    constellation.save()

    serializer = ConstellationSerializer(constellation)
    data = serializer.data

    assert data["bot_default_ignore_meshtastic_portnums"] == "345,ROUTING_APP"
    assert data["bot_default_meshtastic_hop_limit"] == 5
