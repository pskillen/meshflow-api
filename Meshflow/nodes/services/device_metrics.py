...
def get_device_metrics_bulk(
    ...
    Returns:
        QuerySet of DeviceMetrics ordered by meshtastic_node_id, reported_time.
    """
    if not node_ids:
        return DeviceMetrics.objects.none()

    qs = DeviceMetrics.objects.filter(node__meshtastic_node_id__in=node_ids).select_related("node")
