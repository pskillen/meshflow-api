...
def schedule_traceroutes():
    """
    Run periodically (e.g. every 2h). For each ManagedNode, pick one target,
    create AutoTraceRoute (trigger_type=auto, trigger_source=scheduler),
    send traceroute command via channel layer.
    """
    channel_layer = get_channel_layer()
    managed_nodes = ManagedNode.objects.filter(allow_auto_traceroute=True).select_related("constellation").all()
    created = 0

    for source_node in managed_nodes:
