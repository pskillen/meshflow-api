...
| **`last_verification_notify_at`** | datetime, null | When a **verification-start** Discord DM was last attempted (see [discord.md](discord.md); can be disabled via env). Used with env cooldown to limit repeat DMs. Cleared with other presence state when the node is heard again. |
