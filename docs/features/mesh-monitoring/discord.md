...
[Mesh monitoring](README.md) can notify users when a **watched** node is **confirmed offline** after a verification round (traceroutes + deadline), and optionally when **verification starts** (monitoring traceroutes are being dispatched).
...
When mesh monitoring **first enters** a new verification episode (silence exceeded threshold, **`verification_started_at`** set, monitoring round dispatched), the API can optionally DM the same watcher audience **before** offline is confirmed. This helps with aggressive **`offline_after`** values or flap debugging (you see that RF verification has started, even if **`_dispatch_monitoring_round`** finds no sources).

| Control | Details |
|---------|---------|
| **Feature flag** | **`MESH_MONITORING_NOTIFY_VERIFICATION_START`** — treated as enabled only when truthy (`1`, `true`, `yes`, `on`, case-insensitive). **Default unset / off** so production stays quiet until operators opt in. |
| **Cooldown** | **`MESH_MONITORING_VERIFICATION_NOTIFY_COOLDOWN_SECONDS`** (default **21600** = 6 hours). If **`NodePresence.last_verification_notify_at`** is set and the last notify is newer than this window, another verification-start DM is **not** sent for the next episode start. |
